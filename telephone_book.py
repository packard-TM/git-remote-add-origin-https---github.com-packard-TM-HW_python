...
fle = Path('telephone_book.db')
fle.touch(exist_ok=True)

db = sqlite3.connect("telephone_book.db")
cur = db.cursor()
...
# функция создаёт окно, которое позволяет создать новую телефонную книгу
def create_new_telephone_book(*args, start=False, dwindow: Tk = None):
    ...
    # функция создаёт телефонную книгу и закрывает окно
    def create(*largs):
    ...
    window = Tk()
    window.title("Создание телефонной книги")
    window.geometry("350x200")

    Label(window, text="Введите название телефонной книги:", font=("Arial", 10)).place(relheight=0.1, relwidth=0.8, relx=0.1, rely=0.1)
    ...
    errorLabel = Label(window, text="Введите название телефонной книги!", foreground="red")

    Button(window, text="Отмена", command=exit, font=("Arial", 10)).place(relheight=0.1, relwidth=0.2, rely=0.8, relx=0.1)

    Button(window, text="Создать", command=create, font=("Arial", 10)).place(relheight=0.1, relwidth=0.2, rely=0.8, relx=0.7)

    window.mainloop()

# функция создаёт окно, которое позволяет добавлять новую запись в телефонную книгу
def add_person(book_id: int, book_name: str, func_update, func_selecting):
    ...
    # функция создает новую запись в телефонной книге и закрывает окно
    def create(*args):
        nonlocal window, errorLabel, first_nameEntry, last_nameEntry, _birthday, _address, work_numberEntry, home_numberEntry, indexEntry
        global db, cur
        ...
        if (first_nameEntry.get() or last_nameEntry.get()) and (work_numberEntry.get() or home_numberEntry.get()):
            cur.execute(
                ...
                (book_id, first_nameEntry.get(), last_nameEntry.get(), _birthday.get(), _address.get(),
    ...
    window = Tk()
    window.title(f"Добавить запись в телефонную книгу \"{book_name}\"")
    ...
    Label(window, text="Дата\nрождения:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05, rely=0.28)
    Label(window, text="Адрес:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05, rely=0.41)
    Label(window, text="Рабочий\nтелефон:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05,
                                                                      rely=0.54)
    Label(window, text="Домашний\nтелефон:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05,
                                                                       rely=0.67)
    Label(window, text="email:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05,
                                                                      rely=0.80)
    ...
    _birthday = Entry(window, font=("Arial", 9), justify=LEFT, validate="key",
                           validatecommand=(window.register(lambda x: change_entry(None, x)), '%P'))
    _birthday.place(relwidth=0.5, relheight=0.1, relx=0.45, rely=0.28)
    _address = Entry(window, font=("Arial", 9), justify=LEFT, validate="key",
                           validatecommand=(window.register(lambda x: change_entry(None, x)), '%P'))
    _address.place(relwidth=0.5, relheight=0.1, relx=0.45, rely=0.41)
...
# функция создаёт окно, которое позволяет изменить запись в телефонной книге
def edit_person(book_name: str, *persons, func_update, func_selecting):
    ...
    # функция изменяет запись в телефонной книге и закрывает окно
    def save(*args):
        nonlocal window, errorLabel, first_nameEntry, last_nameEntry, _birthday, _address, work_numberEntry, home_numberEntry, indexEntry
        global db, cur
        ...
        if (first_nameEntry.get() or last_nameEntry.get()) and (work_numberEntry.get() or home_numberEntry.get()):
            cur.execute(
                ...
                (first_nameEntry.get(), last_nameEntry.get(), _birthday.get(), _address.get(),
    ...
    window = Tk()
    window.title(f"Изменить запись в телефонной книге \"{book_name}\"")
    window.geometry("300x450")
    ...
    _birthday.insert(0, persons[4])
    _address.insert(0, persons[5])
...
# функция создаёт окно, в котором можно посмотреть список записей в одной из телефонных книг
def active_book(book_id: int, book_name: str, create_date: str):
    ...
    def selecting(*args):
        nonlocal personsListbox, editButton, deleteButton, firstnameEntry, lastnameEntry, birthday, addressEntry, worknumberEntry, homenumberEntry, sorted_persons, flag
        ...
        if personsListbox.curselection():
            ...
            if birthday['state'] == "disabled":
                birthday.configure(state="normal")
            if addressEntry['state'] == "disabled":
                addressEntry.configure(state="normal")
            ...
            birthday.insert(0, person[4])
            addressEntry.insert(0, person[5])
    ...
    # функция вызывает функцию, которая позволяет изменить запись в телефонной книге
    ...
    # функция удаляет запись из телефонной книге
    def delete_person():
    ...
    # функция делает поиск записей в телефонной книге и cортирует их по имени или фамилиии
    ...
    # функция обновляет список записей в телефонной книге
    def update():
    ...
    # функция не даёт вставить значения в поле, где отображается информация о записи в телефонной книге
    def validate():
        nonlocal flag
    ...
    window = Tk()
    window.title(f"Телефонная книга \"{book_name}\"")
    ...
    file_menu.add_command(label="Выбрать другую телефонную книгу", command=lambda: exit(select_book))
    file_menu.add_command(label="Создать новую телефонную книгу", command=lambda: create_new_telephone_book(dwindow=window))
    ...
    Button(window, text="Поиск:", font=("Arial", 10), command=sort_and_filter).place(relwidth=0.15, relheight=0.035, rely=0.03, relx=0.55)
    ...
    Label(frame, text="Дата\nрождения:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05, rely=0.31)
    Label(frame, text="Адрес:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05, rely=0.44)
    ...
    Label(frame, text="email:", font=("Arial", 10)).place(relwidth=0.35, relheight=0.1, relx=0.05, rely=0.83)
    ...
    birthday = Entry(frame, font=("Arial", 9), justify=LEFT, validate="key", validatecommand=(window.register(validate), ), state="disabled")
    birthday.place(relwidth=0.5, relheight=0.1, relx=0.45, rely=0.3)
    addressEntry = Entry(frame, font=("Arial", 9), justify=LEFT, validate="key", validatecommand=(window.register(validate), ), state="disabled")
    addressEntry.place(relwidth=0.5, relheight=0.1, relx=0.45, rely=0.44)
...
# функция создаёт окно, в котором можно переименовать телефонную книгу
def rename_book(book_id, book_name, book_last_edit_date, func_update, func_select):
    ...
    # функция изменяет название телефонной книги и закрывает окно
    def save(*args):
    ...
    window = Tk()
    window.title(f"Переименование телефонной книги \"{book_name}\"")
    window.geometry("350x200")

    Label(window, text="Введите новое название телефонной книги:", font=("Arial", 10)).place(relheight=0.1, relwidth=0.8, relx=0.1, rely=0.1)
    ...
    Button(window, text="Сохранить", command=save, font=("Arial", 10)).place(relheight=0.1, relwidth=0.2, rely=0.8, relx=0.7)

    window.mainloop()

# функция создаёт окно, в котором можно выбрать телефонную книгу
def select_book():
    def exit():
        nonlocal window
    ...
    # функция вызывается каждый раз при выборе телефонной книги из списка
    def selecting(*args):
    ...
    #  функция вызывает функцию, которая позволяет переименовать телефонную книгу
    ...
    # функция удалет телефонную книгу
    def delete_book():
        ...
        result = askyesno(title=f"Удаление телефонной книги \"{book[1]}\"", message=f"Вы уверены, что хотите удалить телефонную книгу \"{book[1]}\"?")

        if result:
            cur.execute("DELETE FROM books WHERE id = ?", (book[0],))
            db.commit()

            showinfo("Телефонная книга удалена", message=f"Телефонная книга \"{book[1]}\" удалена навсегда")

            booksListbox.delete(select_book)
            selecting()

    # функция обновляет список телефонных книг
    def update():
    ...
    window = Tk()
    window.title("Выбор телефонной книги")
    window.geometry("350x200")

    Label(window, text="Выберите телефонную книгу:", font=("Arial", 10)).place(relheight=0.1, relwidth=0.8, relx=0.1, rely=0.05)
    ...
    Button(window, text="Выбрать", font=("Arial", 10), command=select).place(relheight=0.1, relwidth=0.2, rely=0.85, relx=0.1)
    ...
    errorLabel = Label(window, text="Выберите телефонную книгу!", foreground="red")

    file_menu = Menu(window, tearoff=0)
    file_menu.add_command(label="Создать новую телефонную книгу",
                          command=lambda: create_new_telephone_book(update, selecting))
    file_menu.add_separator()
    file_menu.add_command(label="Выход", command=exit)
...
def main():
    ...
    if books == []:
        create_new_telephone_book(start=True)
    else:
        select_book()
